...
def _guess_corner_block(ws: Worksheet) -> Tuple[int, int, int, int]:
    max_row = min(ws.max_row or 1, 200)
    max_col = min(ws.max_column or 1, 40)

    targets = {"科目", "考生姓名", "考生考号", "考生班级学号", "考场", "考场号", "座位号"}
    ...
    for r in range(1, max_row + 1):
        for c in range(1, max_col + 1):
            ...
            if not isinstance(v, str):
                continue
            if v.strip() not in targets:
                continue
            r0 = r
            c0 = c
            r1 = min(max_row, r0 + 25)
            c1 = min(max_col, c0 + 10)
            found = set()
    ...
    if best is None:
        return (1, 1, min(ws.max_row or 1, 40), min(ws.max_column or 1, 12))
    return best
...
def inspect(path: Path) -> Dict[str, Any]:
    ...
    for rng in sorted(ws.merged_cells.ranges, key=lambda r: (r.min_row, r.min_col)):
        ...
        if isinstance(v, str) and v.strip() and ("考试" in v or "台角纸" in v):
            title_merge = rng
            title_value = v.strip()
            break
    ...
    if title_merge is not None:
        ...
        if next_title is not None:
            ...
            r1 = min((ws.max_row or r0), r0 + 40)
    else:
        r0, c0, r1, c1 = _guess_corner_block(ws)
        title_value = None
    ...
    for row in range(r0, r1 + 1):
        for col in range(c0, c1 + 1):
            ...
            b = info.border
            has_border = any(
                getattr(getattr(b, side, None), "style", None)
    ...
    return {
        ...
        "title": title_value,
        "corner_guess": {"r0": r0, "c0": c0, "r1": r1, "c1": c1},
...
def main() -> None:
    xlsx = Path(r"d:\百度网盘同步空间\BaiduSyncdisk\自动化办公\智能考务系统\测试文件\台角纸_批量生成.xlsx")
    out = inspect(xlsx)
