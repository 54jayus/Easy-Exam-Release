...
class StudentInfoTablePDFGenerator:
    ...
    def generate(self, progress_callback=None):
        ...
        if not group_order:
            layout = self._compute_layout(doc.height, 30, include_summary=True)
            table_data = [self._title_row(title_value, len(headers), font_name), headers]
            table_data += [[""] * len(headers) for _ in range(30)]
            table_data.append(self._count_row("", 0, len(headers)))
            row_heights = [layout["title_h"], layout["header_h"]] + [layout["body_h"]] * 30 + [layout["summary_h"]]
