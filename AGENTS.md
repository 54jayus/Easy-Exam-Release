...
All local runtime configuration is centralized in the project-root files:

- Template: `.env.runtime.example`
- Machine-local config: `.env.runtime.local` (gitignored)

Do not hardcode machine-specific Python or Conda paths in documentation or code comments. When you need to reference how to run backend, tests, or packaging, point to `.env.runtime.local` and the helper scripts in `tools/` instead.

Key Python dependencies: `pandas`, `openpyxl`, `xlrd`, `xlsxwriter`, `PyInstaller`
