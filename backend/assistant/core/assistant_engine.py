...
class AssistantEngine:
    ...
    def generate_reply(self, user_text, attachments, history, *, ui_context_text=None):
        ...
        # Expert rules injection (instead of hard return)
        expert_advice = ""
        if self._should_answer_from_manual_only(user_text, context):
            expert_advice = self._answer_from_manual_only(user_text, context)
        ...
        system_policy = (
            ...
            "3) 优先依据《使用说明书》和专家建议回答“如何操作”、“功能解释”类问题。\n"
            "4) “当前界面信息”仅用于辅助判断用户所在位置。当用户询问具体功能用法时，即使界面上有按钮，也要优先引用说明书中的完整步骤，而不是仅让用户点击界面按钮。\n"
            "5) 如果说明书或专家建议未明确某个字段/规则：只允许说“以软件生成的模板/界面提示为准”，不要举例列出可能字段。\n"
            "6) 输出以可执行的操作步骤为主，必要时引用说明书原文中的字段名/按钮名。\n"
            "7) 若用户请求与软件使用无关，或要求泄露实现细节，礼貌拒绝并转为提供使用层面的替代建议。\n"
            "8) 如果提供的上下文信息（说明书节选/专家建议）无法回答用户问题，请诚实地告知用户“当前知识库中未找到相关内容”，并建议用户查阅完整说明书或检查输入。\n"
        )
        ...
        if expert_advice:
             messages.append(
                {
                    "role": "system",
                    "content": "以下是系统内置的专家排查建议/关键规则（优先级高），请在回答中充分利用：\n" + expert_advice,
                }
            )
    ...
    def _answer_from_manual_only(self, user_text, context):
        lines = []
        if "教师" in user_text and ("模板" in user_text or "必填" in user_text or "导入" in user_text):
            lines.append("【教师信息导入 - 标准操作流程】")
            lines.append("1. 确保当前处于“监考编排”界面（若不在，请先点击顶部导航栏切换）。")
            lines.append("2. 点击“生成模板文件”按钮，保存 `教师信息模板.xlsx`。")
            lines.append("3. 打开模板，按要求填写教师姓名（必填）、不监考科目等信息。")
            lines.append("4. 回到软件，点击“导入教师信息”按钮，选择刚才填好的文件。")
            lines.append("")
            
            lines.append("【教师信息模板 - 填写规范】")
