...
from .generators.excel.exam_bag_label import ExamBagLabelGenerator
...
class GeneratorFactory:
    ...
    def create_generator(config: BaseConfig):
        ...
        elif isinstance(config, ExamBagLabelConfig):
            if export_pdf and not export_xlsx:
                return ExamBagLabelPDFGenerator(config)
            return ExamBagLabelGenerator(config)
        else:
